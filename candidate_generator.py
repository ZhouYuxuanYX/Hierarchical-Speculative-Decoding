...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def get_candidates(self, input_ids: torch.LongTensor) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        ...
        # but during my forward sampling process of backward algorithm, there will be no bonus tokens
        new_cur_len = input_ids.shape[-1]

        max_new_tokens = min(int(self.num_assistant_tokens), self.generation_config.max_length - new_cur_len - 1)

        # print("check max new tokens")
        # print(max_new_tokens)

        min_new_tokens = max(min(max_new_tokens, self.main_model_min_length - new_cur_len), 0)
        # print("check min new tokens")
        # print(min_new_tokens)
        if max_new_tokens == 0:
        ...
        # print(self.generation_config.assistant_confidence_threshold)
        # it is by default used, see "_get_stopping_criteria()" function, this causes the draft model to early stop
