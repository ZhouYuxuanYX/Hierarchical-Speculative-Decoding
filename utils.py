...
class _CudaTimer:
    def __init__(self):
        ...
        self.ms = 0.0
    def __enter__(self):
        torch.cuda.synchronize(); self.e0.record(); return self
    def __exit__(self, *exc):
        self.e1.record(); torch.cuda.synchronize()
        self.ms += self.e0.elapsed_time(self.e1)
...
class GenerationMixin:
    ...
    def generate(
        ...
        total_time_dict['1'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['2'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['3'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['4'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['5'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['6'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['7'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['8'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['9'] = end_time - start_time
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            end_time = time.perf_counter()
            total_time_dict['11'] = end_time - start_time

            # 12. run assisted generate
            start_time = time.perf_counter()
            result, counts, total_time_dict = self._assisted_decoding(
                input_ids,
            ...
            end_time = time.perf_counter()
            total_time_dict['12'] = end_time - start_time
        ...
        end_time = time.perf_counter()
        total_time_dict['cache'] = end_time - start_time

        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            # # # print("yes")
            return result, counts, total_time_dict
        else:
            # # # print("no")
            return result
    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        prefill_timer = _CudaTimer(); prefill_timer.__enter__()  # prefill starts now
        _prefill_closed = False
        verify_ms = 0.0
        hsd_cpu_ms = 0.0
        assistant_ms = 0.0

        # reset assistant accumulator from candidate_generator.py
        if hasattr(candidate_generator, "_assist_ms"):
            candidate_generator._assist_ms = 0.0
        else:
            candidate_generator._assist_ms = 0.0
        ...
        # init attention / hidden states / scores tuplesget_candidates
        scores = () if (return_dict_in_generate and output_scores) else None
        raw_logits = () if (return_dict_in_generate and output_logits) else None
        decoder_attentions = () if (return_dict_in_generate and output_attentions) else None
        cross_attentions = () if (return_dict_in_generate and output_attentions) else None
        ...
        in_count = 0
        total_time_dict['inloop'] = 0
        total_time_dict['inloop_1'] = 0
        total_time_dict['inloop_21'] = 0
        total_time_dict['inloop_22'] = 0
        total_time_dict['inloop_23'] = 0
        total_time_dict['inloop_3'] = 0
        total_time_dict['inloop_41'] = 0
        total_time_dict['inloop_42'] = 0
        total_time_dict['inloop_5'] = 0
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            depth = 0
            hist_lengths = [0]
            start_inloop = time.perf_counter()
            while inner_loop:
                ...
                start_inloop_1 = time.perf_counter()
                if not first_step:
                    if not recursive:
                ...
                end_inloop_1 = time.perf_counter()
                total_time_dict['inloop_1'] += end_inloop_1 - start_inloop_1
                # 2. Use the original model to obtain the next token logits given the candidate sequence. We obtain
                # `candidate_length + 1` relevant logits from this process: in the event that all candidates are correct,
                # we use this forward pass to also pick the subsequent logits in the original model.

                # 2.1. Prepare the model inputs
                start_inloop_21 = time.perf_counter()
                if not first_step:
                    ...
                    if "num_logits_to_keep" in model_inputs:
                        ...
                        # # print(model_inputs["num_logits_to_keep"])
                end_inloop_21 = time.perf_counter()
                total_time_dict['inloop_21'] += end_inloop_21 - start_inloop_21

                # 2.2. Run a forward pass on the candidate sequence
                start_inloop_22 = time.perf_counter()
                # prepare variable output controls (note: some models won't accept all output controls)
                model_inputs.update({"output_attentions": output_attentions} if output_attentions else {})
                model_inputs.update({"output_hidden_states": output_hidden_states} if output_hidden_states else {})
                with _CudaTimer() as _verify_t:
                    outputs = self(**model_inputs)

                verify_ms += _verify_t.ms
                if not _prefill_closed:
                    prefill_timer.__exit__(None, None, None)
                    _prefill_closed = True

                # # # print("output shape")
                # # print(outputs.logits.shape)
                end_inloop_22 = time.perf_counter()
                total_time_dict['inloop_22'] += end_inloop_22 - start_inloop_22
                # 2.3. Process the new logits
                start_inloop_23 = time.perf_counter()
                if not first_step:
                    ...
                    if len(logits_processor) > 0:
                        for i in range(candidate_length + 1):
                            ...
                            new_logits[:, i, :] = logits_processor(candidate_input_ids[:, : cur_len + i],
                                                                   new_logits[:, i, :])
                end_inloop_23 = time.perf_counter()
                total_time_dict['inloop_23'] += end_inloop_23 - start_inloop_23
                # 3. Select the accepted tokens. There are two possible cases:
                # Case 1: `do_sample=True` and we have logits for the candidates (originally from speculative decoding)
                # 👉 Apply algorithm 1 from the speculative decoding paper (https://arxiv.org/pdf/2211.17192.pdf).

                start_inloop_3 = time.perf_counter()
                ...
                end_inloop_3 = time.perf_counter()
                total_time_dict['inloop_3'] += end_inloop_3 - start_inloop_3
                hsd_cpu_ms += (time.perf_counter() - start_inloop_3) * 1000.0
                ...
                start_inloop_41 = time.perf_counter()
                input_ids = torch.cat((input_ids, valid_tokens), dim=-1)
                if streamer is not None:
                    streamer.put(valid_tokens.cpu())
                new_cur_len = input_ids.shape[-1]
                end_inloop_41 = time.perf_counter()
                total_time_dict['inloop_41'] += end_inloop_41 - start_inloop_41


                # 4.2. Discard past key values relative to unused assistant tokens
                start_inloop_42 = time.perf_counter()
                ...
                end_inloop_42 = time.perf_counter()
                total_time_dict['inloop_42'] += end_inloop_42 - start_inloop_42
                ...
                start_inloop_5 = time.perf_counter()
                ...
                end_inloop_5 = time.perf_counter()
                total_time_dict['inloop_5'] += end_inloop_5 - start_inloop_5

                if this_peer_finished:
                    break
            

            end_inloop = time.perf_counter()
            total_time_dict['inloop'] += end_inloop - start_inloop
        ...
        if not _prefill_closed:
            # generation ended without a target forward; close prefill window
            prefill_timer.__exit__(None, None, None)
        counts["timing_prefill_ms"]   = float(prefill_timer.ms)
        counts["timing_assistant_ms"] = float(assistant_ms)
        counts["timing_verify_ms"]    = float(verify_ms)
        counts["timing_hsd_cpu_ms"]   = float(hsd_cpu_ms)
        if streamer is not None:
            streamer.end()
        ...
        if return_dict_in_generate:
            ...
            return input_ids, counts, total_time_dict
...
def _speculative_sampling(
    ...
    if backward:
        ...
        elif n_matches < candidate_length-hist_length and p_primes[:, n_matches].sum()==0:
            n_matches +=1
