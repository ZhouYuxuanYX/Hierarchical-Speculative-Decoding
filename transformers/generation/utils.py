...
class GenerationMixin:
    ...
    def prepare_inputs_for_generation(
        ...
        if self._supports_cache_class:
            ...
            # print("target model input cache position")
    ...
    def _get_candidate_generator(
        ...
        """
        # It only checks that all three variables — assistant_model, target_tokenizer, and assistant_tokenizer — are not None.
        different_tokenizers = all(v is not None for v in (assistant_model, target_tokenizer, assistant_tokenizer))

        # # # print("different_tokenizers")
        # # print(different_tokenizers)
    ...
    def generate(
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result, counts = self._assisted_decoding(
                input_ids,
    ...
    def _sample(
        ...
        if parallel:
            for _ in range(multigen - 1):
                input_ids = torch.cat([input_ids, input_ids[0].unsqueeze(0)], dim=0)

                unfinished_sequences = torch.cat([unfinished_sequences, unfinished_sequences[0].unsqueeze(0)],
                                                 dim=0)

        while self._has_unfinished_sequences(
                this_peer_finished, synced_gpus, device=input_ids.device, cur_len=cur_len, max_length=max_length
        ):
            # for multidraft generation starting from the same node at each position
            # because my multidraft is based on the first draft, always expand to additional nodes
            # at each position of the first draft
    ...
    def _assisted_decoding(
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        # for tracking the selected draft in multidraft mode to correctly match past_key_values
        selected_draft = 0
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            inner_loop = True
            ...
            while inner_loop:
                ...
                if not first_step:
                    ...
                    candidate_input_ids, candidate_logits = candidate_generator.get_candidates(input_ids,
                                                                                               multidraft=multidraft, selected_draft=selected_draft if multidraft>1 else None, parallel=parallel)

                    # print("candidate logits shape")
                    # print(candidate_logits.shape)

                    # print("candidate string")
                    # for i in range(multidraft):
                    #     print(tokenizer.decode(candidate_input_ids[i][original_len-5:]))
                    ...
                    if "cache_position" in candidate_kwargs:
                        ...
                        candidate_kwargs["cache_position"] = torch.cat(
                            (
                                ...
                                torch.arange(new_cur_len, new_cur_len + forward_candidate_length,
                                             device=input_ids.device,
                                             dtype=torch.long),
                            ),
                            dim=0,
                ...
                if not first_step:
                    ...
                    new_logits_ = outputs.logits[:,
                                  -forward_candidate_length - 1:].float()  # excludes the input prompt if present
                ...
                if backward:
                    ...
                    if not first_step:
                        ...
                        if return_probs:
                            # n_matches is not equal to accepted_length if the max_new_tokens is reached according to huggingface implementation
                            valid_tokens, n_matches, step_back_probs, p_next, q_next, ids, selected_draft = _speculative_sampling(
                                candidate_input_ids,
                                candidate_logits,
                                candidate_length,
                                new_logits,
                ...
                input_ids = torch.cat((input_ids[:batch_size], valid_tokens), dim=-1)
                if streamer is not None:
                ...
                # print("cache position 0")

                # print(model_kwargs["cache_position"])
                # has to remove the unselected draft for target model for my multidraft algorithm as well!!!!
                outputs.past_key_values = _crop_past_key_values(self, outputs.past_key_values, new_cache_size, selected_draft=selected_draft if multidraft>1 else None)
                # print("cache position 1")
            ...
            if return_probs:
                ...
                counts["q_i"].append(q_next)
                counts["ids"].append(ids)
            ...
            if return_dict_in_generate:
                newly_added_length = new_cur_len - cur_len
                if output_scores:
...
def _forward_sampling(
    ...
    """
    # candidate length is the length of the full historical draft
    new_candidate_input_ids = candidate_input_ids[:, -candidate_length:]
    # get the probabilities from the logits. q_i and p_i denote the draft and target probabilities of the draft tokens
    q = candidate_logits.softmax(dim=-1) if "mps" not in str(
        candidate_logits.device) else candidate_logits.softmax(
        dim=-1)
    ...
    p = new_logits.softmax(dim=-1) if "mps" not in str(new_logits.device) else new_logits.softmax(dim=-1)
...
def _speculative_sampling(
    ...
    if backward:
        ...
        for b in range(multidraft):

            if parallel:
                # check if the prefix is the same
                if (candidate_input_ids[ind:ind+1, :-(candidate_length - n_matches)] == candidate_input_ids[b:b+1, :-(candidate_length - n_matches)]).all():
                    ind = b
                else:
                    continue
            else:
                # for stiped tree, the multidraft is actually the K value, 2 stands for 11 paths
                ind = n_matches * (multidraft - 1) + b


            new_candidate_input_ids = candidate_input_ids[ind:ind + 1, -(candidate_length - n_matches):]
            ...
            if b > 0:
                ...
                p_new = p[ind:ind + 1, n_matches:candidate_length].clone()
                ...
                p_i = p_new[:, torch.arange(candidate_length - n_matches), new_candidate_input_ids].squeeze(1)
                ...
                q_next = joint_q_previous * q[ind:ind + 1, n_matches:candidate_length]
                ...
                # else:
                # p_i corresponds to marginal probability
                p_i = p[ind:ind + 1, torch.arange(n_matches, candidate_length), new_candidate_input_ids].squeeze(1)
            ...
            # flip reverse the order in dim -1, max takes the maximum index(length) where not step back is true
            # torch.flip(~step_back, [-1]).max(-1, keepdim=True)[1] this is not stable, because if all stepback poistions are true
            # it will unexpectedly stop at the last position
            if step_back.all():
                stop_positions = 0
            else:
                stop_positions = candidate_length - n_matches - 1 - torch.flip(~step_back, [-1]).max(-1, keepdim=True)[1]
            ...
            probability_ratio = torch.exp(torch.log(p_i).cumsum(1) - torch.log(q_i).cumsum(1)).unsqueeze(-1)
            ...
            if n_matches>0:
                if n_matches == candidate_length or stop(candidate_input_ids[ind:ind + 1, :n_matches - candidate_length], scores=None):
                    break
        ...
        if is_done_candidate[ind:ind+1] and n_matches == candidate_length:
            ...
            valid_tokens = candidate_input_ids[ind:ind + 1, -candidate_length:]

        else:
            # Next token selection: if there is a rejection, adjust the distribution from the main model before sampling.
            gamma = candidate_length
            p_n_plus_1 = p[ind:ind+1, candidate_length, :]
            if n_matches < gamma:
            ...
            # because if n_matches=0, we add one resampled token for sure, if n_matches=10, we add one more for sure
            # as well, because the previous if checked not stop and n_matches-candidate_length will be 0 causing problem
            if n_matches > 0 and n_matches<candidate_length:
                valid_tokens = candidate_input_ids[ind:ind + 1, -candidate_length:n_matches-candidate_length]
                if not stop(candidate_input_ids[ind:ind + 1, :n_matches-candidate_length], scores=None):
        ...
        if not return_probs:
            ...
            return valid_tokens, n_matches, step_back_probs.cpu().numpy().tolist(), p_i.cpu().numpy().tolist(), q_i.cpu().numpy().tolist(), new_candidate_input_ids.cpu().numpy().tolist(), ind
            ...
            if token_index < candidate_length:
                ...
                if is_accepted:
                    ...
                    if is_done_candidate:
                        valid_tokens = new_candidate_input_ids[:, :token_index]
                        n_matches = token_index - 1
                    else:
                        valid_tokens = torch.cat([new_candidate_input_ids[:, :token_index], chosen_token], dim=-1)
                        n_matches = token_index
        ...
        for b in range(multidraft):
            ...
            current_step_match = ((~is_accepted).cumsum(dim=-1) < 1).sum()
            n_matches += current_step_match
            ...
            if n_matches < candidate_length:
                if b > 0:
                    p_n_plus_1 = p_new[:, current_step_match]
                else:
                    p_n_plus_1 = p[ind:ind + 1, n_matches, :]

                q_n_plus_1 = q[ind:ind+1, n_matches, :]

                p_prime = torch.clamp((p_n_plus_1 - q_n_plus_1), min=0)
                p_prime.div_(p_prime.sum())
            else:
                p_prime = p[ind:ind+1, n_matches, :]
